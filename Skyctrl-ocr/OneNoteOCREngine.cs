...
{
    class OneNoteOCREngine: IOCREngine
    {
        #region Constats
        ...
        public string Recognize(Image image)
        {
            ...
            try
            {
                ...
                result = retrieveText(document, nameSpace, pageId,imageSize); //Retrieves OCR text from updated xml.
            }
            catch (Exception e)
            {
                result = e.Message;
        ...
        private string retrieveText(XDocument document, string nameSpace, string pageId,long imageSize)
        {
            string result;
            int total = 0;
            do // Controlled loop to give some time to OneNote to process the contents of the image
            {
                ...
                int fileSize = Convert.ToInt32(imageSize / 1024 / 1024);
                Thread.Sleep(300 * (fileSize > 1 ? fileSize : 1));
